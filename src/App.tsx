...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<'section1' | 'section2' | 'section3' | 'section5'>(
    'section1'
  );
  ...
  const checkStageValidation = (): boolean => {
    const { isValid, invalidIndices } = validateLessonPlanStages(lessonData.stages);
    if (!isValid) {
      const stageListText = invalidIndices.map((i) => `Stage #${i + 1}`).join(', ');
      alert(
  ...
  const handleExportPdf = async () => {
    ...
    try {
      ...
      showToast('Failed to export PDF. Please try browser print instead.');
    } finally {
  ...
  const handleLoadSample = () => {
    if (window.confirm('Replace current document with completed CELTA sample lesson plan?')) {
  ...
  const handleReset = () => {
    if (window.confirm('Clear all fields to start a blank lesson plan?')) {
    ...
    reader.onload = (event) => {
      try {
        ...
        if (parsed.metadata && parsed.aims) {
          ...
          alert('Invalid lesson plan JSON format.');
        }
      } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-blue-950 text-slate-100 flex flex-col font-sans relative selection:bg-amber-400 selection:text-slate-950">
      ...
      <div
        ...
        aria-hidden="true"
      >
        <DocumentPageLayout data={lessonData} isInteractiveCanvas={false} />
      </div>
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-4 sm:p-6 lg:p-8">
        {/* Toast Notification */}
        {toastMessage && (
          <div className="fixed bottom-5 right-5 z-50 bg-slate-900 text-white px-4 py-2.5 rounded-lg shadow-xl border border-slate-700 flex items-center gap-2 text-xs font-medium animate-in fade-in slide-in-from-bottom-3 no-print">
        ...
        <div className="md:hidden flex items-center justify-center bg-emerald-950 p-1.5 rounded-xl text-white mb-4 no-print shadow-xs">
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-1.5 py-2 text-xs font-semibold rounded-lg ${
              viewMode === 'editor' ? 'bg-amber-600 text-slate-950 font-bold' : 'text-slate-300'
            }`}
          >
          ...
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-1.5 py-2 text-xs font-semibold rounded-lg ${
              viewMode === 'preview' ? 'bg-amber-600 text-slate-950 font-bold' : 'text-slate-300'
            }`}
          >
          ...
          <div className="space-y-6 no-print">
            ...
            {activeTab === 'section1' && (
              <SectionPage1
                ...
                onChangeMetadata={(metadata) => setLessonData({ ...lessonData, metadata })}
                aims={lessonData.aims}
                onChangeAims={(aims) => setLessonData({ ...lessonData, aims })}
              />
            )}

            {activeTab === 'section2' && (
              <SectionPage2
                languageAnalysis={lessonData.languageAnalysis}
                onChangeLanguageAnalysis={(languageAnalysis) =>
                  setLessonData({ ...lessonData, languageAnalysis })
                }
                skillsFocus={lessonData.skillsFocus}
                onChangeSkillsFocus={(skillsFocus) =>
                  setLessonData({ ...lessonData, skillsFocus })
                }
              />
            )}
            ...
            {activeTab === 'section3' && (
              <SectionPage3Stages
                ...
                onChangeOverallAim={(overallAim) => setLessonData({ ...lessonData, overallAim })}
                stages={lessonData.stages}
                onChangeStages={(stages) => setLessonData({ ...lessonData, stages })}
                targetLengthMins={lessonData.metadata.lengthMins}
              />
            )}
            ...
            {activeTab === 'section5' && (
              <SectionPage5Feedback
                ...
                onChangeFeedback={(feedback) => setLessonData({ ...lessonData, feedback })}
              />
            )}
          </div>
        )}
          ...
          <div className="py-4">
            <div className="mb-4 bg-emerald-50 border border-emerald-200 rounded-xl p-3 text-xs text-emerald-950 flex items-center justify-between no-print">
              <div className="flex items-center gap-2">
                ...
                <span>
                  <strong>Interactive Document Canvas:</strong> You can edit text directly inside any field on the document sheets below or click <strong>Export PDF</strong> above for a 1-click clean export.
                </span>
              </div>
            </div>

            <DocumentPageLayout data={lessonData} onChangeData={setLessonData} />
          </div>
        )}
        ...
        {viewMode === 'split' && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 py-2">
            ...
            <div className="lg:col-span-6 space-y-6 no-print">
              <div className="bg-slate-900/90 rounded-xl shadow-md border border-slate-800 p-1.5 flex flex-wrap gap-1">
                {[
                  { id: 'section1', label: '1. Overview', icon: User },
                  { id: 'section2', label: '2. Language', icon: Languages },
                  { id: 'section3', label: '3. Stages', icon: Layers },
                  { id: 'section5', label: '4. Feedback', icon: ShieldCheck },
                ].map((tab) => {
                  const Icon = tab.icon;
                  const isActive = activeTab === tab.id;
                  return (
      ...
      <BrandingSettingsModal
        ...
        onUpdateBranding={(branding) => setLessonData({ ...lessonData, branding })}
      />
    </div>
  );
}
